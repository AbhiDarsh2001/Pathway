...
app.post('/authWithGoogle', async (req, res) => {
    ...
    try {
        ...
        if (!existingUser) {
            const result = await User.create({ name, email, password: ' ', isAdmin: false });
            const token = jwt.sign({ email: result.email, id: result._id }, process.env.JSON_WEB_TOKEN_SECRET_KEY, { expiresIn: '1h' });
            return res.status(200).send({ user: result, token, msg: "User Registered and Logged in Successfully!" });
        } else {
            const token = jwt.sign({ email: existingUser.email, id: existingUser._id }, process.env.JSON_WEB_TOKEN_SECRET_KEY, { expiresIn: '1h' });
            return res.status(200).send({ user: existingUser, token, msg: "User Logged in Successfully!" });
        }
    } catch (error) {
        console.log(error);
...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign({ _id: user._id, email: user.email }, process.env.JSON_WEB_TOKEN_SECRET_KEY, { expiresIn: '1h' });
        res.json({ token, user });
    } catch (error) {
